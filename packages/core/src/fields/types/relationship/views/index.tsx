        ...
        {value.kind === 'many' && (
          <TagGroup
            ...
            onRemove={(keys) => {
              ...
              const item = key ? value.value.find((item) => item.id === key) : null
              if (item) {
                onChange?.({ ...value, value: value.value.filter((i) => i.id !== item.id) })
              }
            }}
            renderEmptyState={() => (
          ...
          >
            {item => (
              <Item href={`/${foreignList.path}/${item.id}`}>
                {item.label}
              </Item>
            )}
          </TagGroup>
...
export function controller (
  ...
  return {
    ...
    filter: {
      Filter: ({ autoFocus, typeLabel, onChange, value }) => {
