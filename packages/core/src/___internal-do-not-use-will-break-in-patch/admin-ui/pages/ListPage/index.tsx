...
function ListPage ({ listKey }: ListPageProps) {
  ...
  return (
    <PageContainer header={<ListPageHeader listKey={listKey} showCreate={allowCreate} />} title={list.label}>
...
const ListPageHeader = ({ listKey, showCreate }: { listKey: string, showCreate?: boolean }) => {
  ...
  return (
    <Fragment>
      <Heading id={LIST_PAGE_TITLE_ID} elementType="h1" size="small">{list.label}</Heading>
      {showCreate && <CreateButtonLink list={list}>{`New ${list.singular.toLocaleLowerCase()}`}</CreateButtonLink>}
    </Fragment>
  )
}
...
`,
  ...
  return (
    <AlertDialog
      ...
      tone="critical"
    >
      Are you sure? This will permanently delete {items.size} item{items.size === 1 ? '' : 's'}.
    </AlertDialog>
  )
}
